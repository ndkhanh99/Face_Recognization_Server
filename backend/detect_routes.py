...
def checkIn(current_user):
    ...
    if student_name != '':
        ...
        if current_user.student_code == name:
            ...
            print(check)
            attend = check or Attendance(subject_id=subject_id,
              lesson_id=lesson_id,
              ...
              time_out='',
              img_out_path='',
              status=u'\u0111i\u1ec3m danh kh\xf4ng \u0111\u1ee7')
            db.session.add(attend)
            db.session.commit()
        else:
            return jsonify({'message': u'\u0110\xe3 \u0111i\u1ec3m danh'})
...
def detectFaces(image_path, file_name, student_folder):
    ...
    count_unknown = 0
    while 1:
        folder = '/Users/macbook/Desktop/attendanceServer/backend/data/raw/1915577'
