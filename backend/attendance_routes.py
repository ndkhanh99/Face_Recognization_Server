    ...
    if current_user.role_id == 3:
        for i in attend_students:
            listAttendance.append({'id':i.id, 
             ...
             'time_out':i.time_out, 
             'status':i.status,
             'img_in_path': i.img_in_path,
             'img_out_path': i.img_out_path
             })
...
@attendance_routes.route('/list/<int:courseId>/<int:lessonId>', methods=['GET', 'OPTIONS'])
@cross_origin()
@token_required
def get_subject_lesson_details(current_user, courseId, lessonId):
    print(id)
    users = Subject.query.filter_by(id=courseId)
    output = []
    for user in users:
        output.append({'id':user.id, 
         'name':user.name, 
    ...
    attend_students = Attendance.query.filter_by(subject_id=courseId, lesson_id = lessonId).all()
    listAttendance = []
    if current_user.role_id == 3:
        for i in attend_students:
            listAttendance.append({'id':i.id, 
